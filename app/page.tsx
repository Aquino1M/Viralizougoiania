 ...
 return <><Header breakingTitle={posts[0]?.title}/><main>
  <section className="heroSection"><div className="container">
   ...
   {featured ? <div className="leadGrid">
     <Link href={`/noticia/${featured.slug}`} className="leadStory"><img src={featured.image_url} alt=""/><div className="leadOverlay"/><div className="leadContent"><div className="storyTag">{featured.category}</div><h1>{featured.title}</h1><p>{featured.excerpt}</p><div className="leadMeta">{featured.city} <span>•</span> {fmt(featured.published_at)}</div></div></Link>
     <div className="leadSide">{secondary.map((p,i)=><Link href={`/noticia/${p.slug}`} key={p.id} className="miniLead"><div className="miniImage"><img src={p.image_url} alt=""/><span>{i===0?"Destaque":"Mais lida"}</span></div><div className="storyTag">{p.category}</div><h2>{p.title}</h2><div className="meta"><b>{p.city}</b><span>•</span>{fmt(p.published_at)}</div></Link>)}</div>
