...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const p=await getPostBySlug(slug);if(!p)return {};return {title:p.title,description:p.excerpt,openGraph:{title:p.title,description:p.excerpt,images:p.image_url?[p.image_url]:[]}}}
...
export default async function Article({params}:{params:Promise<{slug:string}>}){
  const {slug}=await params;const found=await getPostBySlug(slug);if(!found){notFound();return null;}const post=found;
  const all=await getPosts();const paras=post.content.split(/\n\n+/).filter(Boolean);const sourceUrl=safeSourceUrl(post.source_url);
  ...
  return <><Header breakingTitle={all[0]?.title}/><main>
    <article className="article">
      ...
      {post.image_url&&<><img className="articleCover" src={post.image_url} alt=""/><div className="imageCaption">Imagem de capa da matéria • Viralizougoiania</div></>}
