...
export default async function Article({params}:{params:Promise<{slug:string}>}){
  ...
  return <><Header breakingTitle={all[0]?.title}/><main>
    <article className="article">
      ...
      {post.image_url&&<>
        <img className="articleCover" src={proxyImageUrl(post.image_url,1600)} alt="" fetchPriority="high" decoding="async"/>
    ...
    {related.length>0&&<section className="section relatedSection"><div className="container">
      ...
      <div className="cardGrid">{related.map(p=><NewsCard key={p.id} post={p}/>)}</div>
    </div></section>}
  </main><Footer/></>;
}
